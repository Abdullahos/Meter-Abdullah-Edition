...
bool _wrap = true;
bool _hardwareSPI;
uint8_t _colstart = OFFSET_COL, _rowstart = OFFSET_ROW, _tft_type, _rotation = 0, _xstart = OFFSET_COL, _ystart = OFFSET_ROW ,_width =128 ,_height =160;

int16_t _TFT_DC;
int16_t _TFT_RST;
int16_t _TFT_CS;    // Software SPI only
int16_t _TFT_SCLK; // Software SPI only
int16_t _TFT_SDATA;  // Software SPI only
...
void TFT_PowerDown(void)
{
	...
	gpio_write(_TFT_DC, LOW);
	gpio_write(_TFT_RST, LOW);	
#ifndef TFT_SPI_HARDWARE
	gpio_write(_TFT_SCLK, LOW);
	gpio_write(_TFT_SDATA, LOW);
	gpio_write(_TFT_CS, LOW);
#endif
}
...
void TFT_Setup(int16_t rst, int16_t dc, int16_t cs, int16_t sclk, int16_t din)
{
	_TFT_RST= rst;
	_TFT_DC = dc;
	_TFT_CS = cs;
...
{
	uint16_t tft_buf[((x_end-x_start+1)*(y_end-y_start+1))];       
	//uint16_t tft_buf[128*160];           
	//copy the array elements to local array to be able to determine its size using sizeof() function
	//as sizeof(image) will equal the size of pointer not the array size
	memcpy(tft_buf , image , sizeof(tft_buf) );
                 
	setAddrWindow(x_start, y_start, x_start+x_end, y_start+y_end);
	uint8_t *ptrbuf = (uint8_t *)tft_buf;
	//as spi in F1C200S can pass only array with sizeof 32 only so we have to divide the array and loop of them using the following equation
	uint16_t divisor = (2 * (((x_end-x_start+1)*(y_end-y_start+1))/32));
	//TFT_DC_SetHigh;
	gpio_write(DC_TFT, HIGH);
	//for(int i=0; i<640; i++)
	for(uint16_t i=0; i < divisor; i++)
	{
		spi_txrx_len( ptrbuf,  sizeof(tft_buf)/ divisor);
		ptrbuf += (sizeof(tft_buf)/ divisor);
	}
}
...
{
	uint16_t tft_buf[width * height];      
	// uint16_t tft_buf[128*160];                
	memset16_safe(tft_buf , color , ARRAY_SIZE(tft_buf) );
                 
	setAddrWindow(x_start, y_start, x_start+width-1, y_start+height-1);

	uint8_t *ptrbuf = (uint8_t *)tft_buf;
	//as spi in F1C200S can pass only array with sizeof 32 only so we have to divide the array and loop of them using the following equation
	uint16_t divisor = ((width * height)/32);
	uint8_t reminder = ((width * height)%32);
	//printf("divisor = %d , reminder = %d \n",divisor ,  reminder )	;
	//TFT_DC_SetHigh;
	gpio_write(DC_TFT, HIGH); 
	...
	if(reminder > 0)	spi_txrx_len( ptrbuf,  reminder);
}
...
{	
	int8_t i, j;
	uint8_t _width_TFT = 160 , _height_TFT = 128;
	uint8_t _CurrentFontWidth = 5 , _FontNumber = 1;
	uint8_t _CurrentFontoffset = 0x20;
	uint8_t _CurrentFontheight = 8;
	uint16_t arr[160][128] = {ST7735_BLACK};
	uint16_t arr1[128*160] = {ST7735_BLACK};
	uint16_t maxsize = 0;
	///////////////////////////////////////
	...
	for (i = 0; i < _CurrentFontWidth; i++) {
		...
		for (j = 0; j < 7; j++, line >>= 1) 
		{
			...
			if (line & 0x01) 
			{arr[x + (i * size)][y + (j * size)] = color;}
			else if (bg != color) 
			{arr[x + i*size][y + j*size] = bg;}
		}
	}
	uint16_t arr2len = 0;
	for(int hh = 0; hh<128; hh++)
	{
		for(int ww = 0; ww<160; ww++)
		{
			if(arr2len < 20480)
			{arr1[arr2len++] = arr[ww][hh];}			
		}
	}
                 
	// setAddrWindow(x, y,x+127, y+159);
	setAddrWindow(x, y,x+25-1, y+25-1);
	//TFT_DC_SetHigh;
	gpio_write(DC_TFT, HIGH);
	//  uint8_t *ptrbuf = (uint8_t *)(arr1 + ((x*128)+y) );
	uint16_t arr2[25*25];
	arr2len = 0;
	//for(int hh = y; hh<50; hh++)
	for(int hh = y; hh<(y+25); hh++)
	{
		for(int ww = x; ww<(x+25); ww++)
		{arr2[arr2len++] = arr1[((hh*160)+ww)];}
	}	
	uint8_t *ptrbuf = (uint8_t *)arr2;

	//as spi in F1C200S can pass only array with sizeof 32 only so we have to divide the array and loop of them using the following equation
	// uint16_t divisor = (((127-25)*(159-25))/32);
	uint16_t divisor = (((25)*(25))/32);
	for(int r=0; r < divisor; r++)
        {
	        spi_txrx_len( ptrbuf,  sizeof(arr2)/ divisor);
			ptrbuf += sizeof(arr2)/ divisor;
        }
}

void tft_drawtext(uint8_t x, uint8_t y, uint8_t *text, uint16_t color, uint16_t bg, uint8_t size)
{
	uint8_t i = 0 , space = ( 2 + (5*size) );
	while(text[i] != '\0')
	{
		tft_drawchar(x , y , text[i++] , color , bg , size);
		x += space;	
		if( (y > 127) || (x > 160) )	break;
	}
}

void tft_fillscreen(uint16_t color)
{
	tft_fillrec(0,0,128,160,   color );	
}
